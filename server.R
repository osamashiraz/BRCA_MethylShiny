...
# FUNCTION 1 - PROBE CATEGORY PLOTS
...
# FUNCTION 2 - GENE PLOTS
makeGenePlot=function(gene="CDH1", compareBy, DoColSplit=T,type="1"){
  print("here1")
  # HELPER FUNCTION 1 - GET PATIENT IDS FOR SELECTED CLINICAL VARIABLE
  ...
  # HELPER FUNCTION 2 - GET COLUMN SPLITTING ORDER
  ...
  # HELPER FUNCTION 3 - GENERATE HEATMAP ANNOTATIONS
  ...
  # HELPER FUNCTION 4 - GET CLINICAL ANNOTATIONS
  ...
  # HELPER FUNCTION 5 - PLOT REGRESSION LINE
  regressionStat <- function (fit) {
  ...
  # GET METHYLATION AND GENE EXPRESSION DATA
  ...
  scatterPlot=function(df,cols=NULL){
    p=ggplot(df,aes(df[,1],df[,2]))+geom_point(aes(color=df[,3]),alpha=0.5,size=4)+theme_classic()+
      ...
      regressionStat(lm(df[,1] ~ df[,2], data = df))
    if(length(cols)>0){ 
  ...
  # GENERATE TSS-GENE EXP SCATTER PLOTS
  if (type == "2"){
    ...
    if (!DoColSplit){
      # MAIN PLOT
      TSSplot=scatterPlot(p2DF[,c("TSS","exp","annot")])+xlab("TSS Methylation (B-Value)")+ylab("Expression (Counts Per Million)")+myTheme(generalFont = 15,axisFont = 10)
      return(TSSplot)
    }else{
      # SUB PLOTS
      TSSsubplots=lapply(unique(p2DF$annot), function(x){
          col=myColors[match(x,unique(p2DF$annot))]
          scatterPlot(p2DF[p2DF$annot %in% x,][,c("TSS","exp","annot")],cols = col)+xlab("TSS Methylation (B-Value)")+ylab("Expression (Counts Per Million)")+
  ...
  # GENERATE GENE BODY-GENE EXP SCATTER PLOTS
  if (type == "3"){
    annot=chooseClinAnnot(clinData,sampleList,compareBy)
    p2DF=data.frame(exp=matCPM[annot[,1],1], TSS=matHM450[annot[,1],1], Body=matHM450[annot[,1],2],annot=annot[,2])
    ...
    if (!DoColSplit){
      # MAIN PLOT
      Bodyplot=scatterPlot(p2DF[,c("Body","exp","annot")])+xlab("Body Methylation (B-Value)")+ylab("Expression (Counts Per Million)")+myTheme(generalFont = 15,axisFont = 10)
      return(Bodyplot)
    }else{
    # SUB PLOTS
    Bodysubplots=lapply(unique(p2DF$annot), function(x){
      col=myColors[match(x,unique(p2DF$annot))]
      scatterPlot(p2DF[p2DF$annot %in% x,][,c("Body","exp","annot")],cols = col)+xlab("Body Methylation (B-Value)")+ylab("Expression (Counts Per Million)")+
        myTheme(generalFont = 8,subtitleFont = 8)
    })
